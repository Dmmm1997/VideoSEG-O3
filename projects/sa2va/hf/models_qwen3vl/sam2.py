...
class SAM2(nn.Module):
    ...
    def language_embd_inference(self, inference_state, language_embd, image_sample_index):
        num_frame = len(language_embd)
        num_obj = len(language_embd[0])
        mask_out = []
    ...
    def language_embd_inference_keyframe_bidirection(self, inference_state, language_embd, center_index=None, lang_embed_injecting_per_frame=False):
        num_obj = len(language_embd[0])

        right_mask_out = torch.tensor([]).to(language_embd[0][0].device)

        mask_out_shot = []
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            frame_mask_out = []
            for obj_idx in range(num_obj):
                _language_embd = language_embd[0][obj_idx][None][None]
                _, _, out_mask_logits = self.sam2_model.add_language_embd(
                    inference_state,
                    center_index,
                    obj_idx + 100,
                ...
                frame_mask_out.append(out_mask_logits)
            frame_mask_out = torch.cat(frame_mask_out, dim=1)
            mask_out_shot.append(frame_mask_out)

            right_mask_out = []
            if lang_embed_injecting_per_frame:
                language_embd_feat = language_embd[0][0][None][None]
            else:
                language_embd_feat = None
            for out_frame_idx, out_obj_ids, out_mask_logits in self.sam2_model.propagate_in_video(inference_state, start_frame_idx=center_index, language_embed = language_embd_feat):
                right_mask_out.append(out_mask_logits)

            if len(right_mask_out) > 0:
                right_mask_out = torch.cat(right_mask_out, dim=0)

            left_mask_out = []
            for out_frame_idx, out_obj_ids, out_mask_logits in self.sam2_model.propagate_in_video(inference_state, start_frame_idx=center_index, reverse=True, language_embed = language_embd_feat):
                left_mask_out.append(out_mask_logits)

            if len(left_mask_out) > 0:
                left_mask_out = torch.cat(left_mask_out, dim=0)
                # Reverse to maintain chronological order: [Start, ..., Center]
                left_mask_out = torch.flip(left_mask_out, dims=[0])

        # Handle edge cases (e.g., center_index is 0 or last frame)
        if len(left_mask_out) > 0 and len(right_mask_out) > 0:
            # Concatenate: Left[:-1] excludes the duplicate center frame from left side
            mask_out = torch.cat([left_mask_out[:-1], right_mask_out], dim=0)
        elif len(right_mask_out) > 0:
            mask_out = right_mask_out
        else:
            mask_out = left_mask_out

        mask_out_shot = torch.cat(mask_out_shot, dim=0)
        return mask_out, mask_out_shot


    def language_embd_inference_bap(self, inference_state, language_embd, frame_indexes, center_index=None):
        num_frame = len(language_embd)
        num_obj = len(language_embd[0])

        if center_index is None:
            center_index = len(frame_indexes)//2
        center_frame_index = frame_indexes[center_index]
        right_direction_index = frame_indexes[center_index:]
        left_direction_index = frame_indexes[:center_index+1]

        right_mask_out = torch.tensor([]).to(language_embd[0][0].device)

        if len(right_direction_index)>0: # no need for reversing
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                for frame_idx in range(len(right_direction_index)):
                    for obj_idx in range(num_obj):
                        _language_embd = language_embd[center_index+frame_idx][obj_idx][None][None]
                        _, _, out_mask_logits = self.sam2_model.add_language_embd(
                            inference_state,
                            right_direction_index[frame_idx],
                            obj_idx + 100,
                            _language_embd,
                            inference=True,
                        )
                ...
                right_mask_out = []
                inference_state["language_embd"] = _language_embd
                for out_frame_idx, out_obj_ids, out_mask_logits in self.sam2_model.propagate_in_video(inference_state, start_frame_idx=center_frame_index):
                    right_mask_out.append(out_mask_logits)
                right_mask_out = torch.cat(right_mask_out, dim=0)

        self.sam2_model.reset_state(inference_state)
        
        if len(left_direction_index)==0: # no need for reversing
            return right_mask_out

        left_mask_out = torch.tensor([]).to(language_embd[0][0].device)
        #reverse
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            for frame_idx in range(len(left_direction_index)):
                for obj_idx in range(num_obj):
                    _language_embd = language_embd[frame_idx][obj_idx][None][None]
                    _, _, out_mask_logits = self.sam2_model.add_language_embd(
                        inference_state,
                        left_direction_index[frame_idx],
                        obj_idx + 100,
                        _language_embd,
                        inference=True,
                    )
            left_mask_out = []
            inference_state["language_embd"] = _language_embd
            for out_frame_idx, out_obj_ids, out_mask_logits in self.sam2_model.propagate_in_video(inference_state, start_frame_idx=center_frame_index, reverse=True):
                left_mask_out.append(out_mask_logits)
        left_mask_out = torch.cat(left_mask_out, dim=0)
        left_mask_out = torch.flip(left_mask_out, dims=[0])
        mask_out = torch.cat([left_mask_out[:-1], right_mask_out], dim=0)
    ...
    def language_embd_inference_withshotmask(self, inference_state, language_embd, image_sample_index):
        num_frame = len(language_embd)
        num_obj = len(language_embd[0])
        mask_out_shot = []
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
    ...
    def language_embd_inference_withshotmask_addlanguageembd(self, inference_state, language_embd, image_sample_index):
        num_frame = len(language_embd)
        num_obj = len(language_embd[0])
        mask_out_shot = []
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            ...
            mask_out = []
            for out_frame_idx, out_obj_ids, out_mask_logits in self.sam2_model.propagate_in_video(inference_state, language_embed=language_embd[frame_idx][obj_idx][None][None]):
                mask_out.append(out_mask_logits)
            mask_out = torch.cat(mask_out, dim=0)
            mask_out_shot = torch.cat(mask_out_shot, dim=0)
        return mask_out, mask_out_shot
