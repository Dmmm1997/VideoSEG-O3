...
class Sa2VAChatModelQwen(PreTrainedModel):
    ...
    def predict_forward(
        ...
        if image is None and video is None and '<image>' not in past_text:
            ...
            input_dict = {}
            if len(video) > 5:
                sample_index = np.linspace(0, len(video)-1, 5, dtype=int)
            else:
                sample_index = np.arange(len(video))
        ...
        for seg_hidden_states in all_seg_hidden_states:
            ...
            # pred_masks = self.grounding_encoder.language_embd_inference(sam_states, [seg_hidden_states] * len(sample_index), sample_index)
            pred_masks = self.grounding_encoder.language_embd_inference(sam_states, [seg_hidden_states] * num_frames, list(range(num_frames)))            
