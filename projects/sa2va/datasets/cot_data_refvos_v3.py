...
def extract_select_dict(text: str) -> Optional[Dict[str, Any]]:
    ...
    regex_pattern = r'<select><VTG>(.*?)</VTG>(.*?)</select>'
...
class COTRefVOSv3(Sa2VABaseDataset):
    ...
    def __init__(self,
                 ...
                 select_number=5,
                 sampled_frames=5,
                 max_temporal_frames_perround=10,
        ...
        self.min_pixel_temporal = 4*28*28  # Increased minimum
        self.max_pixel_temporal = 32*28*28  # Increased maximum

        self.min_pixel_spatial = 4*28*28   # Reduced minimum
        self.max_pixel_spatial = 128*28*28  # Reduced maximum

        self.min_pixel_key = 4*28*28  # Reduced maximum
        self.max_pixel_key = 512*28*28  # Reduced maximum
        ...
        self.first_round_temporal_max_frames = sampled_frames # 第一轮最大temporal frames的数量

        if self.arch_type == 'qwen':
            self.IMG_CONTEXT_TOKEN = '<|image_pad|>'
            self.TEMP_CONTEXT_TOKEN = '<|temporal_pad|>'
            self.SPATIAL_CONTEXT_TOKEN = '<|spatial_pad|>'
            self.KEY_CONTEXT_TOKEN = '<|key_pad|>'
            ...
            if len(object_masks) == 0:
                if len(ret_masks) != 0:
                    ...
                    _object_masks = np.zeros(
                        (self.select_number, image_size[0], image_size[1]), dtype=np.uint8)
            else:
                _object_masks = []
    ...
    def prepare_data(self, index):
        ...
        data_dict = self.dataset_map_fn(selected_video_objects, select_k=self.select_number)
        ...
        if data_dict.get('images', None) is not None:
            ...
            image_token_str = self._create_token_string(image_data['num_image_tokens'], len(images), self.SPATIAL_CONTEXT_TOKEN)
            video_token_str = self._create_token_string(temporal_image_data['num_image_tokens'], len(temporal_images), self.TEMP_CONTEXT_TOKEN)

            image_data_total = copy.deepcopy(image_data)
            image_data_total["spatial_frame_tokens"] = image_data["num_frame_tokens"]
            ...
            for round in data_dict["multi_round_frames"]:
                ...
                key_frame_data = self._process_multiple_images([key_frame], min_pixels=self.min_pixel_key, max_pixels=self.max_pixel_key)
                key_token_str = self._create_token_string(key_frame_data['num_image_tokens'], 1, self.KEY_CONTEXT_TOKEN)
                ...
                image_data_total["key_frame_tokens"] = key_frame_data["num_frame_tokens"]
                ...
                image_data_total['g_pixel_values'].extend(key_frame_data['g_pixel_values'])

            out_data_dict.update(image_data_total)
            # Create video token string

            assert len(image_data_total['g_pixel_values']) == len(data_dict['masks'])
            
            # Process conversations using unified method
            conversations = self._process_conversations_for_encoding(
                data_dict['conversations'], image_token_str_list, is_video=True, exp=selected_video_objects['exp']
            )
            ...
            if self.arch_type == 'qwen' and 'num_frame_tokens' in image_data_total:
                conversations = self._expand_video_tokens(
                    conversations, image_data_total["spatial_frame_tokens"], image_data_total["temporal_frame_tokens"], image_data_total['key_frame_tokens'],  image_data_total['num_image_tokens']
                )
    ...
    def dataset_map_fn(self, data_dict, select_k=5):
        ...
        selected_frame_indexes = np.array(data_dict["sample_frame_indice"], dtype = int)
        
        selected_frame_indexes.sort()
        selected_frame_masks_indexes = copy.deepcopy(selected_frame_indexes.tolist())
        ...
        if len_frames > self.first_round_temporal_max_frames + 1:
            selected_temporal_frame_indexes = np.linspace(0, len_frames - 1, self.first_round_temporal_max_frames, dtype=int)
        else:
        ...
        for round_id in range(len(data_dict["cot_trace"])):
            ...
            selected_frame_masks_indexes.append(key_frame_indice)
        ...
        for anno_id in anno_ids:
            ...
            frames_masks_ = []
            for frame_idx in selected_frame_masks_indexes:
        ...
        ret = {'images': images, 'temporal_images': temporal_images, 'conversations': data_dict["conversations"], 'masks': masks, 'multi_round_frames': multi_round_frames, 'selected_temporal_frame_indexes': selected_temporal_frame_indexes, "selected_frame_indexes": selected_frame_indexes}
        return ret
    ...
    def _process_conversations_for_encoding(self, conversations: List[Dict], image_token_str: Optional[str] = None, 
                                          is_video: bool = False, exp = None) -> List[Dict]:
        """
        ...
        for msg in conversations:
            if msg['from'] == "system":
                ...
                if conv_idx==0:
                    value = image_token_str[0][0] + f"\nHere are {len(image_token_str[0][2])} low-resolution frames in this video (frame indice is {str(image_token_str[0][2].tolist())})\n" + image_token_str[0][1] + f"Here are {len(image_token_str[0][3])} high-resolution frames in this video (frame indice:{str(image_token_str[0][3].tolist())}).\n" + value
                    value = value.strip()
                else:
                    value_info = image_token_str[conv_idx][0] + f"\nHere are {len(image_token_str[conv_idx][2])} low-resolution frames in this video (frame indices are {str(image_token_str[conv_idx][2])})\n" + image_token_str[conv_idx][1] + f"Here is key frames (frame indice:{str(image_token_str[conv_idx][3])}).\n"
                    if exp is not None:
                        value_info = value_info + f"The text query is '{exp}'"
                    value = value.replace('<image>', value_info)
                conv_idx+=1
                # input_text += value
    ...
    def _expand_video_tokens(self, conversations: List[Dict], num_spatial_tokens: int, num_temporal_tokens:int, num_key_tokens:int, num_total_tokens: int) -> List[Dict]:
        """
        ...
        if conversations and self.arch_type == 'qwen' and hasattr(self, 'patch_token') and self.patch_token == 1:
            ...
            for ind, conv in enumerate(conversations):
                ...
                input_str = input_str.replace(self.KEY_CONTEXT_TOKEN, self.IMG_CONTEXT_TOKEN * num_key_tokens)
...
if __name__ == '__main__':
    ...
    COTRefVOS_dataset = COTRefVOSv3(
        image_folder="/root/paddlejob/workspace/env_run/daiming/project/MyPub/VideoRL/Sa2VA/data/video_datas/revos",
        expression_file="/root/paddlejob/workspace/env_run/daiming/project/MyPub/VideoRL/DataConstruction/datapipeline/revos_new/generation_data/5-COT_final_with_images.json",
