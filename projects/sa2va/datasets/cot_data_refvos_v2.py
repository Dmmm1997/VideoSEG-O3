...
class COTRefVOSV2(Sa2VABaseDataset):
    ...
    def __init__(self,
        ...
        self.round_max_frames = round_max_frames
        assert expression_file and tokenizer
        # Adjusted pixel ranges to match Qwen3VL expectations
        self.min_pixel_temporal = 16*28*28  # Increased minimum
        self.max_pixel_temporal = 32*28*28  # Increased maximum
        ...
        if self.arch_type == 'qwen':
            ...
            self.TEMPORAL_CONTEXT_TOKEN = '<|temporal_pad|>'
            self.ROUND_CONTEXT_TOKEN = '<|round_pad|>'
    ...
    def prepare_data(self, index):
        ...
        data_dict = self.dataset_map_fn(selected_video_objects, select_k=self.select_number, temporal_max_frames=self.sampled_frames, round_max_frames=self.round_max_frames)
        ...
        if data_dict.get('images', None) is not None:
            ...
            for img_path in data_dict['temporal_images']:
                full_img_path = os.path.join(self.image_folder, img_path)
                image = self._read_image(full_img_path)
                if image is None:
                    raise Exception(f"Failed to read image: {full_img_path}")
            ...
            image_data = self._process_multiple_images(images, min_pixels=self.min_pixel_spatial, max_pixels=self.max_pixel_spatial)

            temporal_image_data = self._process_multiple_images(temporal_images, min_pixels=self.min_pixel_temporal, max_pixels=self.max_pixel_temporal)
            # select SAM image

            image_token_str = self._create_token_string(image_data['num_image_tokens'], len(images), self.IMG_CONTEXT_TOKEN)
            video_token_str = self._create_token_string(temporal_image_data['num_image_tokens'], len(temporal_images), self.TEMPORAL_CONTEXT_TOKEN)

            image_data_total = copy.deepcopy(image_data)
            image_data_total["round_frame_tokens"] = 0

            image_data_total['image_grid_thw'] = torch.cat((image_data_total['image_grid_thw'], temporal_image_data['image_grid_thw']), dim=0)
            image_data_total['pixel_values'] = torch.cat((image_data_total['pixel_values'], temporal_image_data['pixel_values']), dim=0)

            image_data_total["num_image_tokens"] += temporal_image_data["num_image_tokens"]
            image_data_total["temporal_frame_tokens"] = temporal_image_data["num_frame_tokens"]
            
            image_token_str_list = [[video_token_str, image_token_str, data_dict["selected_temporal_frame_indexes"], data_dict["selected_frame_indexes"]]]
            for round in data_dict["multi_round_frames"]:
                round_frames, key_frame, sample_index = round

                # read round frames
                round_frames = [self._read_image(os.path.join(self.image_folder, img_path)) for img_path in round_frames]
                round_frames_data = self._process_multiple_images(round_frames, min_pixels=self.min_pixel_spatial, max_pixels=self.max_pixel_spatial)
                temporal_token_str = self._create_token_string(round_frames_data['num_image_tokens'], len(round_frames), self.ROUND_CONTEXT_TOKEN)

                image_data_total['image_grid_thw'] = torch.cat((image_data_total['image_grid_thw'], round_frames_data['image_grid_thw']), dim=0)
                image_data_total['pixel_values'] = torch.cat((image_data_total['pixel_values'], round_frames_data['pixel_values']), dim=0)
                image_data_total["round_frame_tokens"] = round_frames_data["num_frame_tokens"]
                image_data_total["num_image_tokens"] += round_frames_data["num_image_tokens"]

                image_token_str_list.append([temporal_token_str, sample_index])
            ...
            if self.arch_type == 'qwen' and 'num_frame_tokens' in image_data_total:
                conversations = self._expand_video_tokens(
                    conversations, image_data_total['num_frame_tokens'], image_data_total["round_frame_tokens"], image_data_total["temporal_frame_tokens"], image_data_total['num_image_tokens']
                )
    ...
    def dataset_map_fn(self, data_dict, select_k=5, temporal_max_frames=50, round_max_frames=3):
        images = []
        temporal_images = []
        ...
        if len_frames > temporal_max_frames + 1:
            selected_temporal_frame_indexes = np.linspace(0, len_frames - 1, temporal_max_frames, dtype=int)
        else:
        ...
        for selected_temporal_frame_index in selected_temporal_frame_indexes:
            frame_id = data_dict['frames'][selected_temporal_frame_index]
            temporal_images.append(os.path.join(data_dict['video_name'], frame_id + '.jpg'))
        ...
        multi_round_frames = []
        for round_id in range(len(data_dict["cot_trace"])-1):
            round = data_dict["cot_trace"][round_id]
            start_frame = round["start_frame"]
            end_frame = round["end_frame"]
            key_frame = round["key_frame"]
            if end_frame-start_frame>round_max_frames-1:
                this_round_sampled_indexes = np.linspace(start_frame, end_frame, 3, dtype=int)
            else:
            ...
            key_frame = os.path.join(data_dict['video_name'], data_dict['frames'][key_frame] + '.jpg')
            multi_round_frames.append([temporal_frames, key_frame, this_round_sampled_indexes])
        ...
        ret = {'images': images, 'temporal_images': temporal_images, 'conversations': data_dict["conversations"], 'masks': masks, 'multi_round_frames': multi_round_frames, "selected_frame_indexes":selected_frame_indexes, "selected_temporal_frame_indexes": selected_temporal_frame_indexes}
        return ret
        ...
        for msg in conversations:
            if msg['from'] == "system":
                ...
                if conv_idx==0:
                    ...
                    value = value.replace('<image>', image_token_str[conv_idx][0] + "\n Here are the {} frames in this video\n".format(image_token_str[conv_idx][1]))
                conv_idx+=1
                # input_text += value
    ...
    def _expand_video_tokens(self, conversations: List[Dict], num_frame_tokens: int, num_round_tokens:int, num_temporal_tokens:int, num_total_tokens: int) -> List[Dict]:
        """
        ...
        if conversations and self.arch_type == 'qwen' and hasattr(self, 'patch_token') and self.patch_token == 1:
            ...
            for ind, conv in enumerate(conversations):
                ...
                input_str = input_str.replace(self.ROUND_CONTEXT_TOKEN, self.IMG_CONTEXT_TOKEN * num_round_tokens)
                input_str = input_str.replace(self.TEMPORAL_CONTEXT_TOKEN, self.IMG_CONTEXT_TOKEN * num_temporal_tokens)
...
if __name__ == '__main__':
    ...
    COTRefVOS_dataset = COTRefVOSV2(
        image_folder="/root/paddlejob/workspace/env_run/daiming/project/MyPub/VideoRL/Sa2VA/data/video_datas/revos",
        expression_file="/root/paddlejob/workspace/env_run/daiming/project/MyPub/VideoRL/DataConstruction/datapipeline/revos/generation_data/qwen3vl235bv2/5-COT_final_with_images_v2.json",
        mask_file='/root/paddlejob/workspace/env_run/daiming/project/MyPub/VideoRL/Sa2VA/data/video_datas/revos/mask_dict.json',
        # image_folder="../DataConstruction/source_data/Sa2VA_dataset/GroundMoRe_RefVOS/train/JPEGImages",
        # expression_file="/root/paddlejob/workspace/env_run/daiming/project/MyPub/VideoRL/DataConstruction/datapipeline/groundmore/generation_data/5-COT_final_withframes.json",
        # mask_file='../DataConstruction/source_data/Sa2VA_dataset/GroundMoRe_RefVOS/train/mask_dict.json',
        repeats=1,
        select_number=5,
        sampled_frames=20,
