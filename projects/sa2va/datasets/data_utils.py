...
def sa2va_collect_fn(
    ...
    has_video = any(inst.get('pixel_values_videos') is not None for inst in instances)
    has_video_pe = any(inst.get('video_grid_thw', None) is not None for inst in instances)
    ...
    if has_video:
        ...
        for pixel_values_per_sample in pixel_values_videos:
            assert isinstance(pixel_values_per_sample, torch.Tensor)
