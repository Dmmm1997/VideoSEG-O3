...
class SAM2(nn.Module):
    ...
    def inject_language_embd_nf_nobj(self, sam_states, language_embd, nf_nobj=None):
        high_res_features = [
    ...
    def language_embd_forward(self, inference_state, language_embd, image_sample_index):
        num_frame = len(language_embd)
        num_obj = len(language_embd[0])
        mask_out = []
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
    ...
    def get_sam2_forward_images(self, images):
    ...
    def get_sam2_embeddings_expand(self, feats, expand_size=1):
        feats_exp = dict(feats)
        feats_exp["backbone_fpn"] = list(feats["backbone_fpn"])
        feats_exp["vision_pos_enc"] = list(feats["vision_pos_enc"])

        if expand_size > 1:
            backbone_fpn_exp = []
            vision_pos_enc_exp = []

            for feat in feats["backbone_fpn"]:
                # feat: [T, C, H, W]
                f = feat[:, None].expand(-1, expand_size, -1, -1, -1).flatten(0, 1)
                # f: [T * expand_size, C, H, W]
                backbone_fpn_exp.append(f)

            for pos in feats["vision_pos_enc"]:
                p = pos[:, None].expand(-1, expand_size, -1, -1, -1).flatten(0, 1)
                vision_pos_enc_exp.append(p)

            feats_exp["backbone_fpn"] = backbone_fpn_exp
            feats_exp["vision_pos_enc"] = vision_pos_enc_exp

        # Step 2: Process the features to output
        _, current_vision_feats, current_vision_pos_embeds, feat_sizes = \
            self.sam2_model._prepare_backbone_features(feats_exp)
...
class SAM2Base(SAM2Base_):
    ...
    def _forward_sam_heads(
        ...
        if language_embd is not None:
            # B N C
            assert sparse_embeddings.size(0) == language_embd.size(0), "spare_embedding:size{}, language_emb:size{}".format(sparse_embeddings.size(), language_embd.size())
